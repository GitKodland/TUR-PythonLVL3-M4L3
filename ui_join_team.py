...
class JoinTeamButton(Button):
    ...
    def __init__(self, team_number):
        ...
        super().__init__(label=f"Takım {team_number + 1}")

    async def callback(self, interaction: Interaction) -> Any:
        user = interaction.user
        print(self.team_number)
        await interaction.user.send(f"{self.team_number + 1} takıma katıldınız", delete_after=30)
        player = Player(user=interaction.user,
                        game=self.view.game,
                        team_number=self.team_number,
                        info_message=await interaction.user.send("Oyuna bağlanıyor..."))
